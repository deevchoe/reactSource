...
class Clock2 extends Component {
    // 클래스형 컴포넌트는 props로 기본 constructor를 호출해야 한다.
    constructor(props){
        super(props)

        this.state = {date:new Date()}     // state를 생성자에서 지정.          프로토타입이 만들어져..?
    }
    ...
    componentWillUnmount(){ // 시스템에 의한 콜백
        ...
        clearInterval(this.timerID);   // setInterval() 해제.      setInterval을 멈추게 하는게 clearInterval
    }
    ...
    render(){
        return(
            <div>
                ...
                <h2>현재 시간은 {this.state.date.toLocaleTimeString()}</h2>     {/* state 사용 */}
            </div>
        );
    }
}
